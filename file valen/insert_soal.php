...
<body id="page-top">
    ...
    <div class="container"> 
        ...
        <select name="id_kategori" class="form-control form-control-sm" required>
            ...
            $queryCategory = "SELECT id_kategori, kategori FROM tbl_kategori";
            $resultCategory = $koneksi->query($queryCategory);

            if ($resultCategory->num_rows > 0) {
                while ($rowCategory = $resultCategory->fetch_assoc()) {
                    echo "<option value='" . $rowCategory['id_kategori'] . "'>" . $rowCategory['kategori'] . "</option>";
                }
            }
            ?>
        </select><br>
        ...
        <select name="id_kelas" class="form-control form-control-sm" required>
            ...
            <?php
            $queryClass = "SELECT id_kelas, kelas FROM tbl_kelas";
            $resultClass = $koneksi->query($queryClass);

            if ($resultClass->num_rows > 0) {
                while ($rowClass = $resultClass->fetch_assoc()) {
                    echo "<option value='" . $rowClass['id_kelas'] . "'>" . $rowClass['kelas'] . "</option>";
                }
            }
            ?>
        </select><br>
        ...
        <select name="id_mapel" class="form-control form-control-sm" required>
            <option value="">Pilih mata pelajaran</option>
            <?php
            $querySubject = "SELECT id_mapel, mapel FROM tbl_mapel";
            $resultSubject = $koneksi->query($querySubject);

            if ($resultSubject->num_rows > 0) {
                while ($rowSubject = $resultSubject->fetch_assoc()) {
                    echo "<option value='" . $rowSubject['id_mapel'] . "'>" . $rowSubject['mapel'] . "</option>";
                }
            }
            ?>
        </select><br>
        ...
        <input type="reset" name="reset" class="btn btn-sm btn-primary" value="Reset">
    </div>
</form>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert data into tbl_soal
    $insertSoalQuery = "INSERT INTO tbl_soal (id_kategori, id_kelas, id_mapel, soal, a, b, c, d, kunci_jawaban, aktif)
                        VALUES ('$id_kategori', '$id_kelas', '$id_mapel', '$soal', '$a', '$b', '$c', '$d', '$kunci_jawaban', '$aktif')";

    // Execute the query
    $resultSoal = mysqli_query($koneksi, $insertSoalQuery);
