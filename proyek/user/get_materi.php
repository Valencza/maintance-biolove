...
<body>
    ...
    <div class="cover">
        ...
        if (isset($_GET['kategori'])) {
            ...
            // Query untuk mengambil nama kategori berdasarkan parameter kategori
            $queryKategori = "SELECT kategori FROM tbl_kategori WHERE kategori = '$kategori'";
            $resultKategori = mysqli_query($koneksi, $queryKategori);

            if (mysqli_num_rows($resultKategori) > 0) {
                $rowKategori = mysqli_fetch_assoc($resultKategori);
                $namaKategori = $rowKategori['kategori'];

                // Tampilkan judul dengan nama kategori
                echo '<p class="judul">Materi ' . $namaKategori . '</p>';
            } else {
                echo '<p class="judul">Materi (Kategori Tidak Ditemukan)</p>';
            }
        } else {
            echo '<p class="judul">Materi (Kategori Tidak Ditemukan)</p>';
        }
    ...
    if (isset($_GET['kategori'])) {
        ...
        $query = "SELECT materi, nama FROM tbl_paketbelajar WHERE id_kategori = (SELECT id_kategori FROM tbl_kategori WHERE kategori = '$kategori')";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo '<h1>' . $nama . '</h1><br><br>';
            // Tampilkan materi di tengah layar
            echo '<div style="text-align: left;">' . $materi . '</div><br><br><br>';

            // Tampilkan tombol Rangkuman sejajar dengan Latihan Soal
            echo '<a class="btn btn-light border-dark" href="get_rangkuman.php?kategori=' . $kategori . '" style="border-radius: 35px; margin-left: 500px;">Rangkuman</a>';

            // Modifikasi tombol Latihan Soal untuk menyertakan parameter kategori
            echo '<a class="btn btn-light border-dark" href="latihan.php?kategori=' . $kategori . '" style="border-radius: 35px; margin-left: 20px;">Latihan Soal</a>';
        } else {
...
if (isset($_GET['kategori'])) {
    ...
    $query = "SELECT materi FROM tbl_paketbelajar WHERE id_kategori = (SELECT id_kategori FROM tbl_kategori WHERE kategori = '$kategori')";
    $result = mysqli_query($koneksi, $query);
