...
<body>
    ...
    if (isset($_GET['kelas'])) {
        ...
        echo '<p class="judul">Materi ' . $kelas . '</p>';
    } else {
...
<span class="ms-5 mt-5 mb-5 kategori semester-container">
    <div class="border border-dark p-3 mb-3">
        ...
        $query = "SELECT k.kategori AS kategori, tp.semester AS semester
                  ...
                  WHERE tp.semester = 'Semester 1'"; // Hanya memilih data Semester 1
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <button class="btn btn-light border-secondary rounded-pill" style="font-size: 20px; font-family: \'Judson\', Helvetica;"><a href="get_materi.php?kategori=' . $kategori . '" style="text-decoration: none; color: black;">&nbsp' . $kategori . '&nbsp</a></button>&nbsp&nbsp&nbsp';
                echo '</span>';
                $counterSemester1++;
            }
    ...
    <div class="border border-dark p-3 mb-3">
        ...
        $querySemester2 = "SELECT k.kategori AS kategori, tp.semester AS semester
                           FROM tbl_paketbelajar tp
                           JOIN tbl_kategori k ON tp.id_kategori = k.id_kategori
                           WHERE tp.semester = 'Semester 2'"; // Hanya memilih data Semester 2
        $resultSemester2 = mysqli_query($koneksi, $querySemester2);

        $counterSemester2 = 0; // Menambahkan variabel counter untuk Semester 2

        if (mysqli_num_rows($resultSemester2) > 0) {
            echo '<br><h4>&nbsp&nbsp&nbspSemester 2</h4><br>';

            while ($rowSemester2 = mysqli_fetch_assoc($resultSemester2)) {
                $kategoriSemester2 = $rowSemester2['kategori'];
                echo '<span class="semester-2" style="margin-bottom: 20px; display: inline-block;">';
                echo '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp <button class="btn btn-light border-secondary rounded-pill" style="font-size: 20px; font-family: \'Judson\', Helvetica;"><a href="get_materi.php?kategori=' . $kategoriSemester2 . '" style="text-decoration: none; color: black;">&nbsp' . $kategoriSemester2 . '&nbsp</a></button>&nbsp&nbsp&nbsp';
                echo '</span>';
                $counterSemester2++;
            }
